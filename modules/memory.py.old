...
def recall(query: str, limit: int = 20) -> list:
    ...
    # ── 1. Candidats FTS5 (pré-filtrage rapide côté SQLite)
    fts_keys  = set(search_memory_fts(query_resolved, limit=50))

    # ── 2. Permanents — toujours injectés quelle que soit la requête
...
def save_inline_memory(record: dict, user_msg: str = '', existing: list = None):
    ...
    if duplicate:
        ...
        # Calcul embedding si disponible
        vec = _embed(f"{record.get('sujet','')} {record.get('predicat','')} {record.get('objet','')}")
        if vec is not None:
            import json as _json
            record['embedding'] = _json.dumps(vec.tolist())
        record.setdefault('registre', 'neutre')
        save_memory(record)
