...
GENERATE_SYSTEM_PROMPT = (
    ...
    "nimm_lire_page(url, ingerer=False) : rend le TEXTE PROPRE d'une page web, sans "
    "menus, bandeaux ni publicités. ingerer=True la verse dans la base de connaissances.\n"
    ...
    "  A UTILISER EN PRIORITE pour toute demande de trajet pieton, itineraire pietonne, "
    "plan de deplacement a pied ou description d'un parcours dans une ville. "
    "Genere une CARTE HTML INTERACTIVE sur fond OpenStreetMap reel (Leaflet.js), "
    "avec une section textuelle accessible pour les lecteurs d'ecran. "
    "waypoints = liste de dicts {address, lat, lon, annotation, color}. "
    "IMPORTANT : 'address' DOIT etre une adresse geocodable par Nominatim, "
    "ex: '6 rue Gager Gabillot, Paris' ou 'rue des Volontaires, Paris' ou "
    "'rue Plumet, Paris'. NE PAS mettre de descriptions comme "
    "'Sortie metro', 'angle', 'entre X et Y', '15e arrondissement' dans address. "
    "Pour les intersections utiliser uniquement le nom de rue principal + ville. "
    "lat/lon optionnels : si fournis ils evitent le geocodage. "
    "route_segments = [{from_idx, to_idx, color, label}] (optionnel). "
    "Retourne le chemin du fichier HTML genere et un resume textuel accessible.\n"
)
...
def _build_prologue(thread_id: str, workdir: str) -> str:
    ...
    pv = (
        "def nimm_read_pdf_visual(path, question='', _tid='%s'):\n"
        "    _data = _nimm_json.dumps({\"path\": path, \"question\": question, \"thread_id\": _tid}).encode()\n"
        "    _req = _nimm_ur.Request(\n"
        "        \"http://localhost:8080/api/coanimm/read_pdf_visual\",\n"
        "        data=_data, headers={\"Content-Type\": \"application/json\"})\n"
    ...
    vg = (
        ...
        "\n"
        "def nimm_lire_page(url, ingerer=False):\n"
        "    _data = _nimm_json.dumps({\"urls\": [url] if isinstance(url, str) else url,\n"
        "        \"ingerer\": ingerer}).encode()\n"
        "    _req = _nimm_ur.Request(\n"
        "        \"http://localhost:8080/api/exa/lire\",\n"
        "        data=_data, headers={\"Content-Type\": \"application/json\"})\n"
        "    with _nimm_ur.urlopen(_req, timeout=120) as _r:\n"
        "        _d = _nimm_json.loads(_r.read())\n"
        "    return _d.get(\"resultats\", []) if not _d.get(\"erreur\") else _d[\"erreur\"]\n"
...
def _log_workflow_run(wf: dict, parametre: str, status: str, message: str,
    ...
    try:
        import modules.coanimm_safety as _safety
        caps = (wf or {}).get('meta', {}).get('capacites', [])
        db.add_coanimm_security_log({
            'status': 'ok' if status == 'ok' else 'erreur',
            'workflow': label,
            'steps': len((wf or {}).get('meta', {}).get('etapes', [])),
            'capabilities': [_safety.CAPABILITY_LABELS.get(c, c) for c in caps],
            'files': files_count,
            'reasons': [] if status == 'ok' else [(message or '')[:200]],
        })
    except Exception as e:
