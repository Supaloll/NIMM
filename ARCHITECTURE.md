...
- Champs : `name` `emoji` `id` `system_prompt` — et éventuellement `owner` (id de profil) pour un **masque privé** : il n'apparaît alors que pour ce compte (`list_masks` le filtre) et `load_mask` le refuse aux autres profils (repli sur `lia.json`, cache indexé par utilisateur). Champ optionnel `voice` : voix TTS de ce masque (ex. `edge:fr-CH-ArianeNeural`) — **prioritaire** : tout fil ouvert avec ce masque utilise cette voix (`_resolveVoice()` dans `app.js`), quel que soit le choix global du sélecteur TTS. La voix globale (Paramètres) ne s'applique qu'aux fils sans masque (potards/standard) et aux masques qui ne déclarent pas de `voice`.
...
|---|---|
| 25/09/2026 (yt-dlp ne périmera plus en silence) | **yt-dlp est le seul paquet de NIMM qui PÉRIME : une version ancienne ne « bugue » pas, elle CESSE de marcher d'un coup, parce qu'il parle à des sites tiers qui changent leurs défenses.** Demande de Laurent : « est-ce que ça peut s'automatiser ? ». Réponse mesurée, pas réflexe : NIMM avait DÉJÀ les deux briques (l'installation silencieuse des embeddings, avec témoin d'état et garde-fou anti-relance ; et `/api/update`, qui nomme les nouvelles dépendances sans les installer) — mais aucune ne traite « ce paquet a VIEILLI ». Choix retenu par Laurent : **le plus prudent — NIMM détecte et PRÉVIENT, et un bouton installe sur son clic.** Rien ne se modifie dans le dos de son propriétaire. [modules/youtube.py] `version_installee()` lit `importlib.metadata` et non `import yt_dlp` : après une mise à jour, un import rendrait encore l'ancienne version gardée en mémoire par Python ; `version_en_memoire()` sert à le DIRE (« active au prochain démarrage »). `maj_disponible(verifier)` interroge le catalogue PyPI (quelques kilo-octets, cache en mémoire de 6 h, aucun état sur disque) et **n'installe rien** — verrouillé par test (aucun sous-processus dans ce chemin). `mettre_a_jour()` lance `pip install --upgrade yt-dlp` avec `sys.executable -m pip` et NON « pip » : le piège des deux Python est déjà documenté dans `modules/memory.py`, et il s'est manifesté en vrai pendant cette session. Délai de 300 s ; rien n'est cassé si ça échoue, l'ancienne version reste et NIMM continue. [main.py] `GET /api/video/maj` (avec `verifier=1` pour forcer) et `POST /api/video/maj/installer`. [frontend] bloc dédié dans le panneau « Mise à jour », `aria-live`, vérification à l'ouverture des Paramètres, bouton « Mettre à jour yt-dlp », détail technique copiable. **DEUX PIÈGES ATTRAPÉS EN INTERROGEANT LE VRAI CATALOGUE**, tous deux verrouillés par test : (1) yt-dlp s'annonce « 2026.08.19 » quand PyPI publie « 2026.8.19 » — la MÊME version ; une comparaison de CHAÎNES l'aurait déclarée plus ancienne qu'elle-même, pour toujours, faisant du bouton un mensonge permanent ; d'où `_version_nombres()` et une comparaison en nombres. (2) « je n'ai pas pu savoir » (panne réseau) n'est PAS « c'est à jour » : `a_jour` vaut alors None et l'interface annonce un état indéterminé. **Règle de conception gravée, non négociable : on ne surveille QUE yt-dlp, jamais tout `requirements.txt`** — une mise à jour automatique du fichier entier ferait sauter des versions épinglées et retéléchargerait des gigaoctets, pour des paquets dont une version ancienne tourne des années sans souci. Cache-busting : `20260925-maj-ytdlp`. Scénario permanent : `test_maj_ytdlp_prevent_sans_installer`. |
| 25/09/2026 (le texte des vidéos) | **Une vidéo ne se lisait qu'en l'ENVOYANT à Gemini — clé payante, 18 Mo maximum en local, et une description en retour, jamais le texte. NIMM lit désormais ses SOUS-TITRES.** Origine : Laurent avait entendu parler d'« Agent Reach » (dépôt GitHub qui « donne des yeux à un agent »). Vérification faite sur le dépôt et comparaison point par point avec le code : Agent Reach n'est pas une bibliothèque mais un INSTALLATEUR-AIGUILLEUR pour agents de code (Claude Code, Cursor), et NIMM en fait déjà les quatre cinquièmes — lire une page web, la recherche, la veille, GitHub, les documents et l'OCR. La comparaison a désigné le seul manque réel : le TEXTE des vidéos. [requirements.txt] `yt-dlp` (déclaré ; importé PARESSEUSEMENT, donc NIMM démarre et le dit en français même s'il manque). [modules/youtube.py] NOUVEAU module : `lire_sous_titres(url, langues)` rend un dictionnaire, jamais une exception — reconnaissance du lien (youtu.be, m.youtube, youtube.com, youtube-nocookie, et lien collé SANS « https:// », ce qui est le cas courant depuis un mobile), fiche de la vidéo sans rien télécharger (`skip_download`, `noplaylist`), choix de la piste (une piste ÉCRITE À LA MAIN passe devant une piste automatique : ponctuation et noms propres ; puis `fr`, `fr-orig`, variantes régionales, puis `en`) et anti-SSRF via `net_guard`, comme toute sortie réseau de NIMM. Deux analyseurs : `json3` (JSON structuré, préféré) et VTT/SRT, volontairement tolérant (numérotation de réplique, en-têtes WEBVTT/Kind/Language, NOTE de traducteur, balises `<c>`, réplique sur plusieurs lignes, entités HTML décodées) — et c'est le CONTENU qui choisit l'analyseur, pas l'étiquette de format, si bien qu'un format mal annoncé ne perd plus la piste. Le texte est regroupé en blocs horodatés d'environ 30 s (`[12:30] …`) : une réplique par ligne rendrait des milliers de lignes pour une conférence. Défaut trouvé en écrivant le test, et corrigé : le bloc se ferme désormais AVANT d'accueillir la réplique suivante — sinon celle qui déclenche la coupure se retrouvait horodatée dans le bloc précédent. `lire_et_verser()` range en plus les sous-titres dans la base de connaissances (permanent, donc interrogeables comme un document) ; un échec de lecture n'écrit RIEN. [core/hub.py] outil `read_video_transcript` (paramètres `url`, `ranger`) : DÉCLARÉ au modèle et EXÉCUTÉ, via `asyncio.to_thread` (yt-dlp est bloquant). [main.py] route gatée `/api/coanimm/video_transcript` + entrée au catalogue CoaNIMM (`video_transcript`, désactivable). [modules/coanimm.py] helper `nimm_video_transcript` + documentation du prologue (insertion par script temporaire : le fichier contient des séquences d'échappement à l'intérieur de littéraux, et il est en CRLF — un outil qui convertit l'un ou l'autre casserait le module). [coanimm_safety.py] capacité « recherche », car le lien sort vers le réseau. [frontend/app.js] étiquette française `read_video_transcript: 'une vidéo'` dans `LIVE_NOMS_OUTILS` — sans elle, un test existant refuse l'ajout, ce qui a été vérifié avant d'écrire. [tests/banc_essai_retenue.py] rangé en `OUTILS_CONSULTATION` (lecture seule). Cache-busting : `20260925-video-sous-titres`. Scénario permanent : `test_lecture_texte_video` (horodatage aller-retour, json3, VTT avec ses pièges réels, regroupement, choix de piste, refus d'une cible interne, et câblage bout en bout — y compris le fait que `yt_dlp` n'est PAS importé en tête). MESURE RÉELLE, pas une promesse : « But what is a neural network? » (3Blue1Brown, 18 min 40 s) → 288 répliques, 18 639 caractères, piste française écrite à la main, accents vérifiés (238 « é », aucun caractère de remplacement). Copie datée : `modules/coanimm.py.bak_avant_video_20260925`. |
| 24/09/2026 (cache-busting et description d'image) | **Deux défauts signalés la veille, corrigés — plus un test réparé.** (1) **Cache-busting automatique remis en marche.** Depuis que `index.html` porte un `?v=...` FIGÉ (session précédente), la substitution de `main.py` cherchait la chaîne nue `/static/app.js"` : elle ne trouvait plus rien, donc l'automatique était mort et seul le numéro écrit à la main décidait — sans qu'aucun test ne puisse voir un oubli, car le test de forme regarde la forme, pas la date réelle. Correctif : [main.py] fonction `_injecter_version_static(html, version='')` — le numéro du démarrage remplace le `?v=` **quel qu'il soit** (expression régulière), avec repli sur la chaîne nue si la page n'en porte aucun ; la route `/` l'appelle. (2) **La description d'image survit au rechargement du fil.** Le message rangé en base ne portait que `Prompt : ...` : au rechargement, l'alt relu était la CONSIGNE — le mensonge corrigé la veille revenait par cette porte. Correctif : [frontend/app.js] `_publierImageDansLeFil` écrit une ligne `Description : <alt>` (ramenée sur une ligne, placée **avant** `Fichier :`, car la reconnaissance du fil exige que cette dernière soit la dernière ligne), `_extractGeneratedImage` lit la description **à part** de la consigne, et `_renderBubble` la préfère ; sans description enregistrée, il le DIT au lieu de se rabattre sur la consigne. (3) **Test des embeddings réparé** (défaut trouvé la veille, dit franchement) : il mesurait l'état de la machine — le paquet `sentence-transformers` étant désormais installé, il rendait `ready` et croyait à un bug — au lieu du garde-fou anti-relance ; le sous-processus force maintenant `paquet_embeddings_present = False`. Deux scénarios permanents ajoutés : `test_cache_busting_automatique_malgre_le_numero_fige` (il **exécute la vraie fonction d'injection**, extraite du source par AST, et éprouve aussi le repli sans `?v=`) et `test_description_image_survit_au_rechargement`. Cache-busting : `20260924-description-image`. **107 scénarios.** |
| 24/09/2026 (image dans le fil) | **Une image créée depuis le menu « + » restait enfermée dans le panneau Images — elle apparaît maintenant dans la conversation en cours, même si aucun message n'a encore été envoyé.** Constat de Laurent : « comme tout passe par le menu, je n'ai plus d'image qui s'affiche dans le chat ; toutes les images sont coincées dans ce menu ». Diagnostic : depuis la fusion du studio dans le panneau Images (28/08), l'entrée `plus-imagegen` du « + » ne fait plus qu'ouvrir ce panneau (`_ouvrirStudio`), et le studio rangeait son résultat dans le panneau et la galerie SANS jamais l'écrire dans le fil — alors que la route `/api/imagerie/generer` acceptait déjà un `thread_id`, que le studio n'envoyait même pas. Correctif : [frontend/app.js] nouvelle fonction `_publierImageDansLeFil(fiche)` — le fil est créé au besoin (`createThread`, exactement le cas « aucun message envoyé »), la bulle utilisateur `🖼️ <consigne>` et la bulle assistant (image, alt, boutons Télécharger / Modifier) sont affichées tout de suite, et les deux messages sont écrits en base au format `🎨 Image générée.` + ligne `Fichier : <nom>` que le fil sait relire (`_extractGeneratedImage`) : l'image revient donc au chargement du fil. L'alt affiché est la **description réellement produite** par la génération, jamais la consigne ; en **fil fantôme**, rien n'est écrit (la promesse « aucune trace » passe avant l'affichage) ; le bouton Modifier ne lit les octets qu'au clic (pas de base64 promené dans la page). Le studio envoie désormais `thread_id` et annonce « affichée dans la conversation en cours (ferme cette fenêtre pour la voir) ». Cache-busting : `20260924-images-dans-le-fil` (app.js et styles.css). Un scénario permanent ancre la chaîne : `test_image_produite_entre_dans_le_fil` (le format persisté est confronté à la reconnaissance **réelle** du code, pas à une copie du test). |
| 17/09/2026 (VSBot — cahier des charges) | **Cadrage du futur module VSBot, agent de lecture et d'écriture de fichiers de code, à côté de CoaNIMM — un ajout, pas un remplacement : CoaNIMM reste tel quel.** Le document `docs/CAHIER_DES_CHARGES_VSBOT.md` consigne d'abord le besoin réel (lire un `.py` depuis le mobile, dans un fil, pour le montrer à un autre profil) puis le diagnostic vérifié dans le code : le chat n'a AUCUN outil de lecture de fichier local (27 outils dans `NIMM_TOOLS`, tous inventoriés) ; `extract_document_text` refuse les fichiers de code (`_EXT_TEXTE` ne contient que `txt`, `md`, `csv` — un `.py` reçoit « Format non pris en charge ») ; et les « Dossiers autorisés » ne concernent que l'ÉCRITURE (le panneau le dit, et `coanimm_safety.py` ne confinait que écritures, suppressions et déplacements — les lectures ne sont pas confinées). Conclusion : la lecture de code n'est possible aujourd'hui que par la porte de service (script Python généré par CoaNIMM, opaque, lent et tronqué) — ce n'est pas un réglage manquant, c'est une fonction absente. Cinq briques décrites avec ce qui existe déjà et ce qui manque : sélecteur Plan/Act (`generate_plan` et `run_generated` existent) ; arborescence cliquable (`op_list_files` réutilisable, mais `explore_directory` n'est pas un listing) ; écriture outillée avec diff avant écriture (aucune brique d'écriture de contenu n'existe, `file_writer.py` n'écrivant que des documents, et aucune coloration syntaxique n'est présente dans l'interface — `difflib` est en revanche inclus dans Python) ; annulation par étape (patron `DELETE /api/chat/{thread_id}/last_assistant`) ; timeline d'exécution (le journal `coanimm_history` ne conserve ni la sortie, ni les fichiers lus, ni la liste des fichiers produits). Socle proposé : des outils NOMMÉS (read_file, list_tree, write_file, replace_in_file) appelés par le modèle plutôt que du code généré, ce qui rend le diff, l'annulation et le confinement directs. Garde-fous (confinement, protection du dossier de l'application, copie de retour arrière, refus des fichiers de clés), accessibilité (structure réelle, diff jamais signalé par la couleur seule) et ordre de mise en œuvre en cinq sessions documentés dans le fichier. Aucun code écrit : `coanimm.py`, `coanimm_ops.py` et `coanimm_safety.py` ne sont pas modifiés. |
| 17/09/2026 (règles de collaboration) | **`.clinerules` restructuré sur le modèle des ateliers NIMM ePub et NIMM Voix : une PARTIE A « Collaboration » (commune aux trois ateliers) et une PARTIE B « Technique de ce projet ».** La Partie A apporte ce qui manquait ici : l'esprit du projet et les deux maximes de Laurent, la gouvernance à 3 niveaux (lire / petite modification sûre / demander avant d'agir, avec « en cas de doute → demander »), le profil de communication (tutoiement, émojis dans les récaps et les titres de sections, jamais dans le code ni dans le corps de la documentation), les règles de livraison (tout ce qui est destiné à Laurent existe en lanceur double-clic), l'état des lieux en début de session et le récap en trois points en fin de session, et l'honnêteté technique (un test qui dément une hypothèse se dit franchement et s'écrit). La Partie B reprend les règles techniques NIMM déjà en place (environnement, encodage, scripts Python, bases par profil, Hub-and-Spoke, ARCHITECTURE.md) et ajoute la section « Niveau 3 dans cet atelier » (opérations sur les bases SQLite, suppressions, push, secrets et clés d'API, installation lourde, modification de `core/hub.py`), la sauvegarde datée avant toute opération sensible, les traitements longs lancés en tâche de fond, et les pièges de commandes constatés dans les ateliers voisins. **Workflow : passage acté au langage naturel** — Laurent décrit son idée en français, Cline lit le code, propose, applique, vérifie et récapitule ; les blocs FIND/REPLACE ne sont plus le mode de travail (ils restent une consigne interne d'édition). Python aligné sur `python` (au lieu de `py`), comme NIMM ePub et NIMM Voix. Copie datée du fichier précédent : `.clinerules.bak_avant_partieA_20260917`. `.clinerules` étant gitignoré chez NIMM, le fichier de règles n'est pas versionné. |
| 13/09/2026 (Hamburger invisible sur iPhone) | **Le bouton des fils (hamburger, en haut à gauche) était invisible dans l'appli iPhone — cliquable, mais pas visible — alors qu'il s'affichait normalement sur Android.** Deux causes réunies, trouvées en comparant les deux plateformes plutôt qu'en regardant le bouton. (1) `apple-mobile-web-app-status-bar-style` valait `black-translucent` : sur iPhone, en appli installée depuis l'écran d'accueil, le contenu est posé SOUS la barre d'état — la barre du haut (≈ 36 px : sa hauteur vient du contenu, `--topbar-h` n'étant défini nulle part) passait donc derrière l'heure et la batterie, le hamburger se trouvant tout à gauche, sous l'horloge. Android dessine sa barre d'état AU-DESSUS de la vue web : aucun recouvrement, d'où un défaut visible sur un seul des deux systèmes. Corrigé en passant à `black` : iOS réserve la place, comme Android. Aucune zone sûre n'est gérée dans le CSS (aucun `env(safe-area-inset-*)`, pas de `viewport-fit=cover`) — correction d'UNE ligne préférée au chantier des safe areas, qui aurait aussi touché le bas de page et les modales. (2) Le SVG du hamburger était le seul bouton de la barre sans dimensions déclarées (le bouton Agenda, lui, déclare `width="1em" height="1em"`) : dans un conteneur flex, un SVG sans taille peut se réduire à zéro sur Safari. Dimensions explicites + `flex: none`. **Troisième défaut trouvé en chemin** : la mise en valeur du hamburger sur mobile (fond, bordure, 36 px — écrite dans l'intention « rendre le hamburger visible et cliquable ») ne s'appliquait JAMAIS : même spécificité, mais la règle générale située plus bas dans le fichier (`background: none; border: none`) l'emportait — et sur mobile il n'y a pas de survol pour révéler le bouton. Sélecteur rendu plus spécifique (`#top-bar #toggle-history`). 6 tests statiques verts. |
| 13/09/2026 (Thème Amstrad CPC) | **Un troisième thème d'interface après le sombre et le clair : « Amstrad CPC 6128 » — noir pur, jaune, blanc, police pixel VT323.** Demande de Laurent (« une interface un peu différente… l'esthétique des vieux Amstrad… ça ne risque pas de tout casser ? »). Réponse mesurée avant d'écrire : le socle visuel tient à des variables CSS (402 usages de `var(--…)`), donc un thème se réduit à un bloc de variables et ne touche AUCUNE logique — conversation, mémoire et rappels sont inchangés, et le retour en arrière est un clic. *[frontend/styles.css]* `[data-theme="cpc"]` déclare le même socle que les deux autres thèmes (plus `--accent-dark`, volontairement absent de `:root`) ; habillage « machine » scopé au thème : coins carrés, aucune ombre ni flou, police du titre ramenée à `--font-ui`, bulle utilisateur et barres de défilement repensées, taille de base à 110 % (VT323 est étroite). **Deux pannes évitées en MESURANT plutôt qu'en regardant** : (1) seize endroits écrivaient `color:#fff` sur un fond `var(--accent)` — invisible sur un accent jaune (contraste 1:1) → nouveau rôle `--on-accent`, blanc dans les thèmes existants (donc apparence inchangée au pixel près) et noir dans le thème Amstrad ; les variables `--danger` et `--on-accent` sont désormais déclarées dans le thème clair aussi, pour qu'aucun thème n'hérite en silence ; (2) le fond de la bulle utilisateur et les onglets de fil sont écrits en dur dans le style de base → repris par des règles scopées au thème. *[frontend/index.html]* la case « Mode clair » devient un sélecteur à trois thèmes (`#theme-select`) ; cache-busting passé à `20260913-amstrad` (le serveur le réécrit de toute façon avec son horodatage de démarrage, donc la vraie invalidation du cache vient du redémarrage). *[frontend/app.js]* `_appliquerTheme()` : étiquette sur `<html>`, `theme-color` de la barre d'état du téléphone, et police rétro posée par le MÊME mécanisme que le sélecteur de police (le choix de l'utilisateur est mis de côté puis restitué en sortant du thème — rien n'est perdu). *[tests/test_coanimm_agentique.py]* nouveau vérificateur permanent `test_themes_complets` : les trois thèmes doivent déclarer les mêmes variables — un thème incomplet n'échoue pas, il HÉRITE en silence la couleur d'un autre thème —, plus texte sur accent protégé et trois thèmes réellement proposés dans le sélecteur. 8 tests statiques verts. |
| 13/09/2026 (Modèles DeepSeek renommés) | **Le nom de modèle DeepSeek utilisé par NIMM n'existait plus au catalogue — vérifié contre la source officielle.** Demande de Laurent : « est-ce qu'on est sur le bon modèle ? ». Le catalogue interrogé en direct (déjà en place : `GET /api/models/deepseek`, `core/engine.py::list_models`) ne liste plus que `deepseek-flash` (= DeepSeek-V4.1-Flash, la doc précise « the corresponding models have been retired » pour l'ancien nom) et `deepseek-v4-pro`. Or NIMM portait `deepseek-v4-flash` **à quatre endroits de production** : `FOURNISSEURS_OPENAI_COMPAT`, `_PROVIDER_DEFAULT_MODEL`, le repli de `call_llm`, et le modèle par défaut d'un profil neuf (`core/database.py::create_user`) — plus une liste conseillée qui proposait encore `deepseek-reasoner`, absent du catalogue. Rien n'était cassé : vérifié en direct, l'API accepte toujours les anciens noms par tolérance (`deepseek-v4-flash` et `deepseek-reasoner` répondent) — mais ils ne désignent plus la version attendue. Corrigé partout, y compris les scripts de test et de diagnostic qui appelaient les modèles en dur ; la liste conseillée propose désormais `deepseek-flash` et `deepseek-v4-pro` (1 M de contexte, sans vision). **Validé par appels réels chez DeepSeek** : les deux modèles répondent et `deepseek-flash` émet bien un `tool_calls` avec les outils de NIMM, mode réflexion coupé (`thinking: disabled`) comme NIMM le fait par défaut. Garde-fou ajouté au test permanent `test_modeles_conseilles` : les deux anciens noms sont interdits dans la liste conseillée — le commentaire qui les citait a dû être déplacé HORS du bloc surveillé, le test lisant aussi les commentaires (défaut trouvé par le test lui-même). 4 tests statiques verts : modèles conseillés, fournisseurs câblés, réflexion DeepSeek, encodage UTF-8. |
| 13/09/2026 (Rappels hors chat) | **Un rappel d'agenda arrive désormais même si l'utilisateur ouvre NIMM sans parler.** *[core/hub.py]* la règle des seuils sort du prompt pour devenir une source unique : `_seuil_rappel(delta, type, emis)` (critique à j-7 / j-2 / j-1, important et normal à j-1) — `build_system_prompt` l'appelle au lieu de porter sa propre copie — plus une nouvelle fonction `get_rappels_a_signaler()` qui liste les rappels à annoncer maintenant, **y compris les retards jamais annoncés** (`retard = True`) : jusqu'ici un rendez-vous manqué passait « périmé » en silence dès le message suivant (`perimer_rappels_depasses` le classe 'perime' sans que l'utilisateur ait rien vu) ; un rappel passé mais déjà annoncé n'est pas relancé, et les « flexible » sans date restent muets. *[main.py]* `GET /api/rappels/a-signaler` (mêmes règles que le chat) et `POST /api/rappels/{id}/vu` (marque le seuil annoncé via `marquer_rappel_emis`). *[frontend]* modale `#rappels-alerte-modal` ouverte automatiquement à l'entrée du profil — `verifierRappelsOuverture()` appelée dans `init()`, après un éventuel déverrouillage PIN : « ✓ C'est noté » marque le seuil annoncé (le chat ne répète plus le rappel), « 📅 Ouvrir l'agenda » ouvre l'agenda sans rien marquer (la modale reviendra), et fermer par ✕ ne marque rien non plus — la relance est volontaire. Pastille chiffrée rouge sur le bouton Agenda de la barre du haut, visible même modale fermée. Validation : matrice des seuils, aller-retour création → signal → « vu » → disparition, cas « retard » et cas « sans date », testés sur une **copie** de `data/nimm_laurent.db` (aucune donnée réelle touchée) ; contrôles G6 ajoutés à `tests/audit_routes.py`. |
| 10/09/2026 (Mémoire privée d'un masque) | **Un masque de jeu peut garder « ses » souvenirs, isolés de la mémoire NIMM.** *[core/database.py]* store fichier `data/souvenirs_masques/<masque>.json` — un fichier par masque, HORS base et hors table `memory` : `get_souvenirs_masque()` / `add_souvenir_masque()`, fragments (message de l'utilisateur + réponse du masque) tronqués à 90 et 110 caractères, 12 conservés, écriture atomique (`.tmp` + `os.replace`). *[core/hub.py]* champ de masque `memoire_du_masque` (`true` = 5 fragments, ou un entier) : `_bloc_souvenirs_masque()` les injecte dans le prompt du masque, AVANT le personnage ; `_masque_souvenir_push()` les enregistre depuis `_add_msg()` — **exception assumée au mode fantôme** (un fil fantôme n'écrit rien sur disque, sauf ce fragment, et seulement si le masque le déclare), balises `%%…%%` retirées avant stockage. *[.gitignore]* `data/souvenirs_masques/` : le dossier entier est ignoré, jamais publié. Circuit validé sur un fil fantôme temporaire (écriture, injection, accroche), puis nettoyé. |
| 10/09/2026 (Venice — outils et étages de modèles) | **Venice devient un fournisseur ordinaire : les outils NIMM passent, mais pas pour tous ses modèles.** *[core/engine.py]* `FOURNISSEURS_OPENAI_COMPAT["venice"]["outils"]` passe à `True` ; nouveau `MODELES_SANS_APPEL_OUTILS` (`venice-uncensored-1-2`, `venice-uncensored-role-play`, `hermes-3-llama-3.1-405b`) et `_modele_sans_appel_outils()` : un modèle qui refuse les appels d'outils repasse par le flux simple `call_llm_stream` — volontairement DISTINCT de `_MODELES_SANS_OUTILS`, qui gouverne AUSSI la température. Sans ce garde, le petit modèle du masque de jeu partait en HTTP 400 « tool_choice is not supported ». **Catalogue Venice sondé en direct** (18 modèles, 3 sondes chacun : contenu cru, humour noir, appel d'outil) et prix réels relevés. Répondent sans bridage ET appellent les outils : `openai-gpt-oss-120b` (0,07 / 0,30 $ par million), `qwen3-235b-a22b-instruct-2507` (0,15 / 0,75), `deepseek-v4-pro` (1,65 / 3,30, contexte 1 M), `deepseek-v4-flash`, `gemma-4-uncensored`, `nvidia-nemotron-3-ultra-550b`. À écarter : `qwen-3-8-27b` et `mistral-small-2603` (se brident, réponse vide), `llama-3.3-70b` (réfléchit tout haut), `hermes-3-llama-3.1-405b` (refuse les outils). *[frontend/app.js]* liste Venice réorganisée en **trois étages** (GPT-OSS 120B, Qwen3 235B, DeepSeek V4 Pro — tous outillés), puis les modèles « jeu » sans outils fiables (uncensored-1-2, role-play, gemma, aion) ; cache-busting `20260910-venice2`. **Validation de bout en bout hors interface** : appel → `search_memory` exécuté pour de vrai → réponse finale correcte (« chauffeur poids lourd »). Le masque de jeu vérifié intact (aucun outil, flux simple). |
| 10/09/2026 (Venice — prompt système masqué) | **Un masque de jeu ne s'incarnait plus sur Venice : Venice ajoutait SON prompt système générique devant celui de NIMM** (doc Venice : « your system prompts are appended to Venice's defaults »). Sur un petit modèle (`venice-uncensored-1-2`), ce préambule neutre prend le dessus et le masque est ignoré — répliques d'assistant serviable, alors que le fil est correctement configuré (`mask_id` et `personality_mode` corrects en base, masque bien servi pour le profil propriétaire). *[core/engine.py]* nouveau `_params_venice(provider)` → `venice_parameters: {include_venice_system_prompt: false}`, posé sur les **trois** charges utiles OpenAI-compatibles (appel direct, flux simple, flux avec outils) ; le 4ᵉ chemin (reprise de réponse) est réservé à DeepSeek/Mistral, donc hors sujet. Non-régression vérifiée : DeepSeek/OpenAI reçoivent un dictionnaire vide. Test A/B direct sur l'API Venice : sans le paramètre → réplique générique, avec → réplique incarnée. **Le paramètre était nécessaire mais pas suffisant** : test comparatif sur le fil réel (prompt NIMM complet ≈ 18 k caractères contre 1,7 k pour le masque) — `venice-uncensored-1-2` **suit les consignes impératives et ignore les descriptions nominales** ; sans consigne explicite il redevient poli, alors qu'un prompt impératif franchement suggestif incarne le personnage. `venice-uncensored-role-play` incarne plus fort mais délire (didascalies, mélange de langues, faits inventés). Déplacer la forme nominale en fin de prompt : aucun effet. *[core/hub.py]* deux champs de masque **opt-in** : `persona_final` (le personnage est répété en toute fin de prompt — « rappel final, prime sur tout ce qui précède ») et `prompt_minimal` (le masque ne reçoit QUE son personnage : ni lexique contractuel, ni agenda et rappels, ni mémoire, ni identité, ni index, ni outils, ni quiz — conséquence assumée : plus de tags %%RAPPEL%% / %%IMAGE%% / %%DOMINANT%% / %%SITUATION%%, donc aucun rappel, aucune image, aucune humeur). Sans ces champs, aucun masque existant n'est affecté. Mesures : le même personnage en tête du prompt est ignoré, répété en fin il incarne le rôle ; et réduit au personnage seul, l'incarnation est MEILLEURE qu'avec l'échafaudage complet (prompt de 19 182 caractères contre 1 189). Troisième champ `memoire_minimale` (avec `prompt_minimal`) : digest mémoire compact placé AVANT le personnage — profil certain et proches, ≈ 300 caractères ; l'index thématique (519 car., qui renvoie à un outil absent) et le dump des tripletets (5 844 car.) sont écartés, mesurés en réel. Masque restructuré en impératif chaud, basculé sur `prompt_minimal` + `memoire_minimale` (masque confidentiel, non versionné). |
| 09/09/2026 (Ariston + Venice) | **Ariston : masque perso à tirage verrouillé par fil** (rattrapage d'une session précédente restée non commitée). Colonne `threads.ariston_config` + `get/set_ariston_config` ; verrous de masque dans `load_settings` (provider_lock / temperature / frequency_penalty) ; routes `/api/masks/ariston/options` et `/api/threads/{id}/ariston/tirage` ; modal de tirage ouverte à la création du fil, badge de relecture, styles `.ariston-select`. **Venice AI branché** — modèles hébergés SANS filtre côté serveur pour les masques de jeu de rôle et d'humour noir. `FOURNISSEURS_OPENAI_COMPAT` (`https://api.venice.ai/api/v1`, défaut `venice-uncensored-1-2`, `outils: False`), catalogue `/models` en direct (117 modèles), dispatch `call_llm`, env `VENICE_API_KEY`, service au catalogue (champ de clé), wallet tirelire, option « 🕶️ Venice — sans filtre » + modèles conseillés (uncensored-1-2, role-play, gemma-4-uncensored, aion-3-0), six listes front complétées, README. Clé posée sur le profil laurent (chiffrée Fernet), compte crédité, validé en réel (catalogue, appel direct et flux du chat). GLM Heretic écarté des conseillés (raisonneur muet). |
| 09/09/2026 (géolocalisation mobile) | **Géolocalisation mobile réparée — la conversion GPS → ville passe par le serveur**. Nominatim n'autorisait plus les appels directs depuis le navigateur (en-tête CORS retiré) : `_getLocation()` échouait en silence et NIMM ne recevait aucune position. Nouvelle route `GET /api/geoloc/reverse` (main.py, reverse geocoding Nominatim avec User-Agent `NIMM/1.0`, cache 120 s) ; `frontend/app.js` appelle cette route au lieu de Nominatim direct ; cache-busting `20260909-geoloc` (le téléphone gardait l'ancien app.js en cache, d'où un premier test encore négatif après redémarrage). Suivi des tests en conditions réelles : BACKLOG \[EN COURS\]. |


| 07/09/2026 (relevé de décisions) | **NIMM dépouille un compte rendu — et la vraie difficulté n'était pas l'extraction**. Premier morceau du volet « travail institutionnel ». [modules/decisions.py] `relever()`, `parser_releve()`, `rendre_markdown()`, `rendre_note_carnet()` ; outil `relever_decisions(titre, texte)` dans `NIMM_TOOLS`. Entrée : le document attaché au fil (pris automatiquement, le modèle n'a pas à recopier un compte rendu entier dans un paramètre) ou un texte fourni. Sortie : un docx à vrais titres via `file_writer` **et** une note dans le carnet du fil. **LE RISQUE, ET LES TROIS GARDE-FOUS** — relever des décisions, c'est interpréter ; sur des notes prises à la volée, c'est reconstituer. Une décision attribuée au mauvais responsable est pire que pas de décision : elle sera relayée et personne n'ira vérifier. (a) Rien n'est deviné — un responsable absent reste « non précisé », et la consigne anticipe explicitement le cas où la déduction paraît légitime (« même si le contexte te semble évident »), parce qu'une interdiction abstraite est exactement ce qu'un modèle contourne de bonne foi. (b) Chaque relevé porte la **citation** du passage dont il vient, copiée mot pour mot : le document est vérifiable sans rouvrir la source. (c) Décisions, actions et informations sont distinguées — un compte rendu mélange les trois. Plus : la troncature au-delà de 30 000 caractères est **annoncée**, un relevé incomplet qu'on croit complet étant le pire des deux mondes ; et `parser_releve` absorbe ce qu'un modèle rend vraiment (JSON encadré de bavardage et de balises) sans jamais lever — un relevé raté ne doit pas casser la conversation. **PRÉSENTATION — et une correction de Fernando le jour même :** j'avais d'abord écarté le tableau en invoquant la lecture en braille. **C'était faux, et il l'a corrigé** : un tableau correctement balisé (`th`, `scope`, ligne d'en-tête marquée) se navigue très bien, et c'est la bonne structure pour des données qui ONT des colonnes. Ce qui se lit mal, c'est un tableau SANS balises — une grille muette. Le document porte donc un **tableau récapitulatif** (Intitulé, Nature, Responsable, Échéance) suivi des **extraits justificatifs** en liste, ces derniers n'ayant pas leur place dans une cellule. **Il a fallu ajouter le support des tableaux à `file_writer`** : ni le HTML ni le DOCX ne les produisaient. HTML : `<thead>`, `<th scope="col">`, `<th scope="row">` sur la colonne identifiante. DOCX : `add_table` + marquage `w:tblHeader` de la ligne d'en-tête — sans lui, Word n'annonce pas les intitulés de colonne et ne répète pas l'en-tête d'une page à l'autre. Ce support bénéficie à tout le projet, `write_file` et les exports compris. Le test permanent vérifie désormais **le balisage**, et non l'absence de tableau. **AU PASSAGE, deux garde-fous ont fait leur travail :** le contrôle du banc a refusé le nouvel outil tant qu'il n'était rangé dans aucun camp, et son relevé d'outils, qui se fiait au NOM des variables, était aveugle au premier outil déclaré à part — rendu générique (tout dict littéral ayant la forme d'un outil compte). 1 test permanent ajouté — 103 scénarios. |
...
| 05/09/2026 (banc de retenue) | **Un banc d'essai pour savoir si coaNIMM demande AU BON MOMENT — le mécanisme du 02/09 n'avait jamais été éprouvé**. Constat : la règle de retenue et `demander_precision` sont verrouillés par trois tests **statiques** (la règle est là, la question est lisible en braille, l'outil coupe la boucle). Aucun ne dit s'il se déclenche à propos. Or il peut échouer de **deux façons opposées qui passent toutes deux ces tests** : les modèles ignorent la règle — rien n'a changé et on s'est raconté une histoire ; ou ils la sur-appliquent — coaNIMM demande une précision à chaque tour, exactement le défaut que la contre-règle était censée empêcher. (1) JEU DE CAS — `tests/cas_retenue.txt`, 30 messages étiquetés `clair` / `ambigu` / `discussion`, trois champs par ligne : étiquette, message, **et la raison de l'étiquette**. Cette troisième colonne n'est pas de la décoration : sans elle, le banc mesure l'opinion de celui qui a étiqueté. Format texte à barres verticales plutôt que JSON — Laurent doit pouvoir en ajouter sans se battre avec des virgules, et le fichier reste lisible sur une plage braille. Règle d'écriture inscrite en tête du fichier : formuler les messages comme quelqu'un qui ne connaît PAS coaNIMM ; une demande reformulée pour « bien marcher » ne mesure que la connaissance qu'a son auteur de l'outil — c'est le biais du créateur décrit dans `docs/note_pour_nando_coanimm_agentivite.md`, et c'est pourquoi les cas de Laurent valent plus que ceux de Nando. (2) HARNAIS — `tests/banc_essai_retenue.py` reconstruit le couple **prompt + outils** exactement tel que le chat l'envoie (`build_system_prompt` + `NIMM_TOOLS`), observe la PREMIÈRE décision du modèle et s'arrête là : aucun outil n'est exécuté, **aucune écriture en base** — ni fil, ni message, ni réglage. Quatre décisions distinguées : demande / agit (outil qui produit ou modifie) / cherche (outil de simple consultation) / répond en texte. Verdicts : sur `clair`, échec seulement si le modèle demande — agir ou répondre sont tous deux acceptés, ce banc ne juge pas la qualité de l'action ; sur `ambigu`, succès seulement s'il demande ; sur `discussion`, échec s'il demande OU s'il déclenche un outil de production. Paramétrable (`--fournisseurs`, `--etiquette`, `--limite`, `--depuis`, `--pause`, `--profil`), rapport écrit **au fil de l'eau** dans `tests/results/` pour qu'une coupure réseau ne perde pas les cas déjà passés, et **mode `--a-sec`** qui vérifie tout le circuit — cas lus, règle bien présente dans le prompt, outil bien déclaré — sans dépenser un centime. (3) `BANC_RETENUE.bat` à la racine : menu à quatre entrées (vérification à sec, les 10 cas ambigus, les 30 cas, les 30 sur deux fournisseurs). Le banc doit être lancé **depuis la machine** : les appels API ne passent pas par l'environnement d'assistance. (4) TEST PERMANENT — `test_banc_retenue_coherent` : le verdict du banc repose entièrement sur le rangement de chaque outil dans un camp (produit/consulte). Le jour où un outil est ajouté à `NIMM_TOOLS` sans être rangé, le banc continue de produire des chiffres — simplement faux, en silence. Le test vérifie que **les 36 outils déclarés** sont rangés, qu'aucun outil rangé n'a disparu, que le jeu de cas est bien formé avec au moins 5 cas par étiquette et une raison non vide, et que le banc n'écrit rien en base. Rapports du banc exclus du dépôt (`.gitignore`). 1 test permanent — 101 scénarios. **Reste à faire : passer la campagne et lire les écarts.** |
...
---
### [EN COURS] Géolocalisation mobile — reverse geocoding corrigé, à éprouver en conditions réelles

Le téléphone (PWA) envoie ses coordonnées GPS à chaque message ; NIMM les convertit en ville via Nominatim et l'injecte dans le contexte (`build_system_prompt`, bloc « Position actuelle »).

**Panne trouvée le 09/09/2026** : Nominatim n'envoie plus d'en-tête CORS aux navigateurs → l'appel direct depuis `_getLocation()` (frontend/app.js) échouait en silence et NIMM ne recevait aucune localisation.

**Correctif appliqué le 09/09/2026** : nouvelle route `GET /api/geoloc/reverse` (main.py) qui fait le reverse geocoding côté serveur avec User-Agent `NIMM/1.0` (conforme à la politique d'usage de Nominatim) + cache mémoire 120 s ; `_getLocation()` (frontend/app.js) appelle cette route au lieu de Nominatim direct ; cache-busting index.html passé à `20260909-geoloc` (le téléphone gardait l'ancien app.js en cache, d'où un premier test encore négatif après redémarrage).

**Tests restants** :
- Depuis le téléphone (Firefox Android, permission localisation accordée) : envoyer « où suis-je ? » et vérifier la commune reçue. À Colmar la réponse sera identique à la mémoire (test non discriminant sur place) → test probant hors de la ville connue, OU contrôle du journal serveur (`nimm.log`) : la requête `GET /api/geoloc/reverse?lat=…&lon=…` doit apparaître à l'envoi d'un message depuis le mobile.
- Vérifier le rechargement du code côté PWA standalone (fermeture/rouverture de l'app) après chaque changement de cache-busting.

Vérifiable dans le code : `main.py:geoloc_reverse` · `frontend/app.js:_getLocation` · `frontend/index.html` (`v=20260909-geoloc`).

---


### [EN COURS] Volet « travail institutionnel »

Retenu par Fernando le 07/09/2026, en quatre morceaux. Le premier est livré.

**1. Relevé de décisions — LIVRÉ le 07/09/2026.** `modules/decisions.py` +
outil `relever_decisions(titre, texte)`. Dépouille un compte rendu ou des
notes de séance : décisions, actions (qui, pour quand), points d'information.
Produit un document à diffuser et une note dans le carnet du fil.

**2. Préparer une réunion** à partir des documents reçus : synthèse, points de
vigilance, questions à poser. Le pendant amont du relevé.

**3. Aider à rédiger un avis** selon la structure attendue, avec une relecture
critique de l'**argumentation** — pas de l'orthographe.

**4. Suivre un texte réglementaire dans le temps** : ce qui a changé depuis la
version précédente. La veille signale les nouveautés, pas les évolutions d'un
texte déjà suivi.

**Sources et sorties arrêtées avec Fernando :** l'entrée est un document reçu
(docx, pdf, texte) ou ses propres notes de séance — **pas d'audio**. La sortie
est un fichier à diffuser **et** une note de carnet — **pas de rappels créés
dans l'agenda**.

**HORS PÉRIMÈTRE, écarté explicitement :** l'audit d'accessibilité de
documents ou de pages web. Ce n'est pas le métier de Fernando et il ne veut pas
de cela dans NIMM. NIMM sait *produire* de l'accessible, il n'a pas vocation à
*vérifier* celle des autres.
...
### [FAIT le 06/09/2026] Réécriture de l'historique du dépôt

Accord de Laurent, opération menée le 06/09/2026. `git filter-repo --replace-text`
sur un clone neuf, six règles, puis `push --force`. Le dépôt public est passé de
`d9d9d74` à `9f89661`.

**Résultat vérifié :** les quatre chaînes visées ont disparu de tout l'historique
(chemin de bureau, nom de fichier client, dossier de travail, chemin de
sauvegarde) ; 339 commits conservés ; et surtout **l'empreinte du contenu actuel
est restée identique à l'octet près** (`3012955a…`) — seuls les vieux commits ont
été réécrits, la version d'aujourd'hui n'a pas bougé.

**Le piège qui a failli tout gâcher, à ne pas reproduire.** Le premier fichier de
règles portait des commentaires. `git filter-repo --replace-text` ne les traite
PAS comme des commentaires : il prend chaque ligne non vide pour une règle, et une
ligne sans `==>` remplace la chaîne par `***REMOVED***`. Les lignes réduites à un
`#` ont donc remplacé **tous les `#` du dépôt** — 138 fichiers touchés :
commentaires Python, titres Markdown, scripts shell. Seuls les `.bat` étaient
intacts, leurs commentaires n'utilisant pas ce caractère. Détecté par le contrôle
d'empreinte du contenu avant toute publication, jamais poussé. **Un fichier de
règles ne contient que des règles.**

**Le contrôle qui a servi**, à refaire pour toute opération de ce genre :
comparer `git rev-parse HEAD:` avant et après. Si le contenu actuel ne contient
aucune des chaînes visées, cette empreinte doit être **strictement identique** —
toute différence signale un remplacement qui a débordé.

**Ce qui reste, et que la réécriture ne règle pas :**
- La **pull request ouverte** (`refs/pull/1/head`) conserve les commits d'origine :
  les refs de PR survivent au push force. À fermer.
- GitHub **garde les objets déréférencés** accessibles par leur hash. Une demande
  explicite au support est nécessaire pour qu'ils soient ramassés.
- Tout clone ou fork antérieur garde l'historique complet — Laurent doit
  re-cloner.
- `ARCHITECTURE.md` mentionne toujours `H:\Mon Drive\…` dans le récit de
  l'incident du 10/08 : c'est **volontaire** (le journal doit pouvoir nommer les
  chemins fautifs) et ce n'est pas un chemin exploitable. Le vrai chemin complet,
  lui, a disparu.
