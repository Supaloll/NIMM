...
| 14/06/2026 | **Diagnostic + correction extraction mémoire (triplets)**. [hub.py] `extract_memories_background()` : prompt d'extraction entièrement réécrit — sujet = prénom du proche (pas de l'utilisateur), faits chiffrés dans l'objet (pas le contexte), définitions des prédicats `trait` vs `probleme_sante` pour éviter les confusions, 2 exemples concrets guidant le LLM. `max_tokens` 300→600. Outil de diagnostic isolé `tests/diag_extraction.py` créé (appel API direct, simulation pipeline sans écriture DB, rapport par message). Score avant : "Laurence" comme sujet fantôme, chiffres perdus dans le contexte. Score après : 23/23 triplets acceptés sur 10 messages tests, Maïssane correctement identifiée sur tous les cas. Cache-busting : `20260614-1`. |
