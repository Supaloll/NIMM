...
| 17/06/2026 | **Chiralite des relations memoire**. [modules/memory.py] Concept de relation horizontale (symetrique) vs verticale (chirale) : `PREDICATS_SYMETRIQUES` definit les seules relations dont le retournement est valide (conjoint, ami, collegue, frere_ou_soeur). `_save_symmetric()` bloque desormais toute reciproque automatique pour les relations verticales (pere/mere, enfant, chef/subordonne, medecin/patient...) — corrige les triplets aberrants `Jean / enfant / Laurent`. [data/prompts/] Les trois prompts provider (deepseek, anthropic, mistral) mis a jour : regle « un seul triplet par fait, dans le sens naturel de l'enonce — jamais la reciproque » — le LLM ne genere plus les deux sens. Diagnostic obtenu en interrogeant NIMM sur son propre comportement. |
