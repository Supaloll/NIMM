...
| 15/06/2026 | **Prompt d'extraction mémoire DeepSeek + injection contexte**. Nouveau fichier `data/prompts/memoire_deepseek.txt` : shadow prompting + chain notation, exemples anonymisés ([H]/[F]), logique esprit-plutôt-que-lettre (rôle principal vs secondaire, réciprocité genrée, quantités sans prénom). [hub.py] `extract_memories_from_window()` injecte désormais `{{DATE}}` et `{{LOCATION}}` dans le template prompt. Cache-busting : `20260615`. |
