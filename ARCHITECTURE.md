...
| 09/09/2026 (Ariston) | **Ariston — masque perso à tirage verrouillé par fil (travail de session précédente, commité ici)**. [modules/masks/ariston.json] nouveau masque privé (`owner: laurent`, `ghost`, `provider_lock: gemini`, `temperature: 1.7`) ; [database.py] colonne `ariston_config` (threads) + `get/set_ariston_config` ; [hub.py] verrous de masque dans `load_settings` (provider_lock / temperature / frequency_penalty), repli tirage aléatoire de secours, `resolve_mask_for_settings` ; [engine.py] `frequency_penalty` propagé (Gemini + flux avec outils) ; [main.py] routes `/api/masks/ariston/options`, `/api/threads/{id}/ariston/config`, `POST …/tirage` ; [frontend] modal de tirage ouverte à la création d'un fil Ariston, badge de relecture, styles `.ariston-select`. |
| 09/09/2026 (Venice) | **Venice AI branché — fournisseur de modèles hébergés SANS filtre côté serveur (RP érotique assumée, humour noir trash)**. [engine.py] `venice` ajouté à `FOURNISSEURS_OPENAI_COMPAT` (`https://api.venice.ai/api/v1`, défaut `venice-uncensored-1-2`, `outils: False`), catalogue `/models` interrogé en direct (117 modèles texte), dispatch `call_llm`, env `VENICE_API_KEY`, ordre de secours ; [hub.py] env + tables `_KEY_MAP` ×2 ; [services.py] service « Venice AI » au catalogue (famille conversation) → champ de clé automatique ; [database.py] wallet tirelire + tarifs 0,20/0,90 $/M ; [frontend] option « 🕶️ Venice — sans filtre » (Réglages + Nouveau fil), `MODELS_BY_PROVIDER.venice` (uncensored-1-2, role-play, gemma-4-uncensored, aion-3-0), les 6 listes en dur complétées ; [README.md] ligne au tableau des providers. Clé posée sur le profil laurent (chiffrée Fernet), compte crédité (2 500 crédits), validé en réel (catalogue, appel direct et flux du chat). GLM 4.7 Heretic écarté des conseillés (raisonneur qui épuise son budget en réflexion invisible). |
