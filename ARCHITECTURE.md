...
| 12/06/2026 | **Intégration patch Nando — mode local, accessibilité clavier, chiralité mémoire**. [hub.py] `load_settings()` : lecture du réglage `local_mode` — si activé, provider basculé sur `ollama`, modèle sur `ollama_model` (défaut `llama3.1:8b`) ; `local_mode` exposé dans le dict retourné. [main.py] Endpoints `/api/settings/local-mode` (GET+POST) et `/api/settings/user-genre` (GET+POST). Propagation `contextvars` aux threads d'enrichissement (correction robustesse). En mode local, `ingest_file` force l'OCR Tesseract (clé Mistral ignorée). [memory.py] Chiralité genrée : réglage `user_genre` (masculin/féminin/vide) affine les réciproques de fratrie (`frere`/`soeur` au lieu du neutre `frere_ou_soeur`). [app.js] Accessibilité : `focusModal()` déplace le focus dans la modale à l'ouverture par raccourci clavier (NVDA suit). Correction gestionnaire `keydown` sur les fils (visait le mauvais élément). Blocs JS mode local et genre utilisateur ajoutés. Dépendances ajoutées : `python-docx`, `trafilatura`, `striprtf`, `mistralai`, `pdf2image`, `playwright` + Chromium headless. Cache-busting : `20260612-1`. |
...
| 13/06/2026 | **Dictée vocale optionnelle et configurable**. [modules/stt.py] `STTModule.__init__` lit `stt_model` depuis la DB (fallback `"base"` si DB inaccessible). [main.py] `_warmup_stt()` vérifie `stt_enabled` avant de charger Whisper — si désactivé, skip silencieux. Endpoints `GET/POST /api/settings/stt` : expose `enabled` (bool) et `model` (tiny/base/small/medium/large). [app.js] IIFE STT : charge les réglages au démarrage et à l'ouverture des settings, masque/affiche `micBtn` (desktop) et `mobile-mic-btn` (mobile) selon l'état, masque le sélecteur de modèle si OFF, sauvegarde à chaque changement. [index.html] Section "🎙️ Dictée vocale (Whisper)" dans les réglages : toggle ON/OFF + sélecteur de modèle avec RAM indicative (390 Mo → 10 Go). Valeur par défaut : ON / base. Cache-busting : `20260613-1`. |
