...
| 25/08/2026 (masque fantôme) | **Masque à mode confidentiel automatique** — un masque peut déclarer `ghost: true` : tout fil qui l'utilise est traité en mode fantôme (aucun message persisté via `_add_msg`, aucune note de carnet, aucune extraction mémoire). [core/hub.py] `_is_ghost_thread()` vérifie désormais aussi le masque du fil (`get_thread` → `load_mask` → champ `ghost`) ; le worker utilise `_is_ghost_thread` pour sauter ces fils. [main.py] `GET /api/threads/{id}/ghost` reflète l'état réel (liste ou masque) ; `POST …/ghost` ne permet pas de désactiver le mode quand le masque l'impose. Création du masque `mlle_liam.json` (💋 Mlle Liam, privé `owner: laurent`, `ghost: true`). |
