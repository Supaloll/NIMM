...
### [DÉCISION DE LAURENT] Réécrire ou non l'historique du dépôt

**Ce qui reste exposé après le ménage du 06/09/2026** : dans deux commits sur
339 (`cea6eb5` qui l'introduit, `70968d8` qui le nettoie), le chemin
`C:\Users\<nom>\Desktop\<dossier>\<fichier client>.svg` et un identifiant de
profil. Plus, entre `b2949e2` et `5a56d54`, le `H:\Mon Drive\...` de
`data/backup_config.json`. Le contenu ACTUEL du dépôt est propre.

Ce n'est pas un secret technique : aucune clé, aucun mot de passe. C'est un nom
de client et une arborescence personnelle. La question est donc de savoir si ce
nom de client justifie l'opération — et c'est à Laurent de trancher, c'est son
nom sur les commits.

**Ce que l'opération coûte** : `git filter-repo` recalcule toute la chaîne
depuis le commit touché, donc tous les hashes changent. Tout le monde doit
re-cloner ou faire un reset dur. À faire quand personne n'a de commit local en
attente — y compris les sessions Claude et Cline.

**Ce qu'elle ne règle pas, et c'est le point à peser :**
- GitHub **conserve les objets déréférencés**, accessibles par leur hash, après
  un push force. Les faire ramasser demande une **demande explicite au support
  GitHub** ; sans elle, les anciens commits restent atteignables par URL.
- Le dépôt porte une **pull request ouverte** (`refs/pull/1/head`). Les refs de
  PR survivent au push force et conservent les commits d'origine.
- Tout clone ou fork déjà réalisé garde l'historique complet.

Autrement dit : l'opération réduit l'exposition, elle ne l'annule pas. Si
l'objectif est que ce nom disparaisse vraiment, la demande au support GitHub
fait partie du travail — sinon on paie le coût du re-clonage général pour un
résultat partiel.

**Si Laurent dit oui**, la procédure tient en quelques commandes (`git
filter-repo --replace-text`, puis `git push --force`), à lancer depuis une
machine et non depuis l'environnement d'assistance : le dossier monté y refuse
la suppression de fichiers, ce dont `filter-repo` a besoin pour réécrire
`.git/`, et un précédent a laissé des verrous `.git/index.lock` impossibles à
effacer après un simple commit lancé de là.
