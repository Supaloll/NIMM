...
| 26/06/2026 | **Migration Gemini 2.0 Flash → 3.5 Flash (modèle 2.0 retiré par Google) + tarifs à jour**. [core/engine.py] `_PROVIDER_DEFAULT_MODEL['gemini']` : `gemini-1.5-flash` (déjà obsolète) → `gemini-3.5-flash` ; fallbacks codés en dur dans `_call_gemini` et `_gemini_tools_turn` (`model = model or 'gemini-2.0-flash'`) → `gemini-3.5-flash` ; URL vision `call_gemini_vision` (`gemini-2.0-flash:generateContent`) → `gemini-3.5-flash:generateContent`. Génération/retouche d'image (`gemini-2.5-flash-image`, Nano Banana) non concernée — modèle distinct, toujours actuel. [frontend/app.js] menu déroulant Gemini : 3 entrées mortes (2.0 Flash, 1.5 Flash, 1.5 Pro) → 2 entrées (`gemini-3.5-flash` économique, `gemini-3.1-pro-preview` puissant). [core/database.py] wallet Gemini : `compteur_requetes`/`daily` (0.075$/0.30$ par M tokens, ancien tarif quasi-gratuit) → `tirelire`/`manual` (1.50$/9.00$ par M tokens, tarif réel 3.5 Flash) dans `default_wallets` et `TARIFS_DEFAUT` — **mise à jour SQL de la base existante (`nimm.db`) à faire séparément**, ce changement ne s'applique qu'aux nouvelles installs (`INSERT OR IGNORE`). Cache-busting : `20260626-1`. |
| 26/06/2026 (suite) | **Migration génération d'images Gemini : Nano Banana → Nano Banana 2, fin du quota gratuit**. [core/engine.py] `_generate_gemini_image` et `edit_gemini_image` : URL `gemini-2.5-flash-image:generateContent` → `gemini-3.1-flash-image:generateContent` (mêmes formats de requête/réponse, simple changement de modèle). [core/database.py] `TARIFS_DEFAUT` : nouvelle entrée `gemini_image` (0.50$ entrée / 60.00$ sortie par M tokens, ≈ 0,067$/image en 1K résolution selon tarif officiel Google) — **préparée mais pas encore branchée** : `_generate_gemini_image`/`edit_gemini_image` n'appellent toujours pas `log_cost`, donc la génération d'image Gemini reste invisible dans le suivi de coût actuel (angle mort existant, pas introduit par cette session). [frontend/index.html] libellé sélecteur génération image : « 🟢 Gemini (1 500 images/jour gratuites) » (faux depuis la bascule Google vers tarification systématique des modèles image, confirmé sur la page officielle de tarification consultée le 26/06/2026 : `gemini-2.5-flash-image` et tous les modèles image Gemini actuels affichent désormais « Free Tier: Not available ») → « 🟠 Gemini (≈ 0,067 $/image, plus de quota gratuit) ». Pas de cache-busting requis (aucun fichier `app.js`/`styles.css` modifié dans ce lot). |
