...
### [PRIORITÉ] Passer le banc d'essai « agir ou demander »

**L'outil existe depuis le 05/09/2026, la mesure non.** `BANC_RETENUE.bat` à la
racine, ou directement `python tests/banc_essai_retenue.py` : 30 cas étiquetés
(`tests/cas_retenue.txt`), aucune écriture en base, rapport dans
`tests/results/`. Commencer par le choix 1 (vérification à sec, gratuit), puis
le choix 2 (les 10 cas ambigus) — c'est là que se joue l'essentiel : coaNIMM
demande-t-il quand il devrait ?

**À lancer depuis la machine**, les appels API ne passant pas par
l'environnement d'assistance.

**Ce qu'on cherche, et les deux échecs possibles :** un taux faible sur
`ambigu` veut dire que les modèles ignorent la règle de retenue — le prompt ne
suffit pas, et le routeur d'intention écarté le 02/09 reprend de l'intérêt. Un
taux élevé sur `ambigu` mais des écarts sur `clair` ou `discussion` veut dire
l'inverse : la règle mord trop, et c'est sa formulation qu'il faut resserrer.
Les deux se corrigent, mais pas au même endroit — d'où l'intérêt de mesurer
avant de toucher à quoi que ce soit.

**Les cas doivent venir de Laurent autant que de Nando.** Le fichier de cas
s'ajoute à la main, une ligne par cas, et porte sa règle d'écriture en tête :
formuler comme quelqu'un qui ne connaît pas l'outil. C'est tout l'objet de sa
note (`docs/note_pour_nando_coanimm_agentivite.md`).
