...
async def call_llm(
    ...
    if provider == 'anthropic':
        ...
        return await _call_openai_compat(messages, model or 'deepseek-chat', system_prompt, max_tokens, temperature, api_keys, 'deepseek', 'https://api.deepseek.com/v1')
    elif provider == 'gemini':
        return await _call_gemini(messages, model, system_prompt, max_tokens, temperature, api_keys)
    elif provider == 'openai':
        return await _call_openai_compat(messages, model or 'gpt-4o', system_prompt, max_tokens, temperature, api_keys, 'openai', 'https://api.openai.com/v1')
    elif provider == 'openrouter':
        return await _call_openai_compat(messages, model or 'mistralai/mistral-7b-instruct', system_prompt, max_tokens, temperature, api_keys, 'openrouter', 'https://openrouter.ai/api/v1')
    elif provider == 'mistral':
        return await _call_openai_compat(messages, model or 'mistral-small-latest', system_prompt, max_tokens, temperature, api_keys, 'mistral', 'https://api.mistral.ai/v1')
...
async def _call_openai_compat(messages, model, system_prompt, max_tokens, temperature, api_keys, provider_name, base_url):
...
async def call_llm_stream_with_tools(
    ...
    Pour les providers non supportés (Anthropic, Gemini, Ollama) :
    ...
    if provider not in _SUPPORTED:
        # Fallback : stream normal sans tools
